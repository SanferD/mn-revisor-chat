...
func Crawl() error {
	...
	rawDataStore, err := stores.InitializeS3Helper(ctx, mySettings.BucketName, mySettings.RawPathPrefix, mySettings.ContextTimeout, mySettings.LocalEndpoint)
	if err != nil {
	...
	if err := application.Crawl(ctx, urlQueue, seenURLStore, rawDataStore, webClient, bgInterruptWatcher, logger); err != nil {
